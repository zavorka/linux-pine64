...
static s32 lcd_edp_delay_ms(u32 ms)
{
...
static s32 lcd_edp_delay_us(u32 us)
{
#if defined(__LINUX_PLAT__)
	udelay(us);
#endif
...
inline void IIC_SCLB_LOW(void)
{
	...
	//	printf("!!!!!!%s\n",__func__);
	uint reg_val;
	reg_val = readl(base_addr + 0x04);	//PH CFG
	reg_val &= ~(0x7 << (LCD_SCK_INDEX_CFG1*4));
	reg_val |= (0x1 << (LCD_SCK_INDEX_CFG1*4));	//SCK
	writel(reg_val, (base_addr + 0x04));
	reg_val = readl(base_addr + 0x10);	//ph4 
	reg_val &= ~(1<<LCD_SCK_INDEX);
	writel(reg_val, (base_addr + 0x10));
...
inline void IIC_SCLB_HIGH(void)
{
	...
	//	printf("!!!!!!%s\n",__func__);
	uint reg_val;
	reg_val = readl(base_addr + 0x04);	//PH4 CFG
	reg_val &= ~(0x7 << (LCD_SCK_INDEX_CFG1*4));
	writel(reg_val, (base_addr + 0x04));
...
inline void IIC_SDAB_LOW(void)
{
	...
	//printf("!!!!!!%s\n",__func__);
	uint reg_val;
	reg_val = readl(base_addr + 0x04);	//PH5 CFG
	reg_val &= ~(0x7 << (LCD_SDA_INDEX_CFG1*4));
	reg_val |= (0x1 << (LCD_SDA_INDEX_CFG1*4));	//SDA
	writel(reg_val, (base_addr + 0x04));

	reg_val = readl(base_addr + 0x10);	//ph5 
...
void edp_lcd_panel_init(u32 sel)
{
    ...
    int lcd_model_name;
		base_addr=ioremap(0x1F02C00,  0x400);
